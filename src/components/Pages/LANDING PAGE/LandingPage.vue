...
export default {
    ...
    methods: {
        ...
        //scroll navigation
        scrollNavigate(id){
            const section = document.getElementById(id)  //animation scrolling if not pushing new route
            if(section){
                const headerHeight    = document.querySelector('.header').offsetHeight;
                const sectionTop = section.getBoundingClientRect().top
                const YLocation = sectionTop + window.scrollY - headerHeight - 150  //ensuring that the target is slight at the center
                window.scrollTo({
                    top: YLocation,
                })                
            }
        ...
        async fromPushtoScrollNavigate(){   //animation scrolling if new tab
            ...
            if (section) {
                ...
                window.scrollTo({
                    top: sectionTop + window.scrollY - headerHeight - 200,
                    behavior: 'smooth',
                });
            }
        },
