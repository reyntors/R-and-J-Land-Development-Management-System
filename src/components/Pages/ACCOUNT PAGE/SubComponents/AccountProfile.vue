    ...
    <div style="margin-top: 1rem;"> 
      ...
      <section class="uploadedID">
        ...
        <progress-loading type="torks" v-if="isUploading"/>
          <div class="box" id="accountProfileCustomerRealtor"> 
            <input type="file" id="idUpload" accept="image/jpeg, image/jpg, image/png" @change="uploadedImg">
          </div>
          <li class="box" v-if="imageURLDisplay!=''">
              <img :src="imageURLDisplay ">
            </li>
...
export default {
  ...
  data(){
    return{
      ...
      image: null,
      imageURLDisplay: '', //var holder to display the image the client uplaoded before saving it to the cloud
      userId: null,
      isUploading: false,
    }
  },
  methods:{
    ...
    uploadedImg(event){
        ...
        if(img){
            console.log(img)
            this.image = img
            const blob = new Blob([img], { type: 'image/png' });    //covnert the image to blob
            const url = URL.createObjectURL(blob)                   //create url for the image uploaded
            this.imageURLDisplay = url                              //display the image into the DOM before uploading to cloud
        }else{
          this.image = null
        }
    },
    cancelUploadImg(){
      this.imageURLDisplay = '';
      this.image = null;
    },
    async uploadToCloud(){
        if(this.image){
            ...
            this.imageURLDisplay = "";  //reset to empty
            this.image = null;          //reset to null
        }else{
